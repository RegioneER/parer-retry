...
public class RestRetryInterceptor implements ClientHttpRequestInterceptor {
    ...
    public RestRetryInterceptor(List<URI> additionalEndpoints, ParerRetryConfiguration retryClient) {
    ...
    public ClientHttpResponse intercept(HttpRequest hr, byte[] bytes, ClientHttpRequestExecution chre)
        ...
        CryptoCallBack callback = new CryptoCallBack(hr, bytes, chre);

        return retryClient.execute(callback);
    }

    private class CryptoCallBack implements RetryCallback<ClientHttpResponse, RestClientException> {

        private final HttpRequest hr;
        byte[] bytes;
        private final ClientHttpRequestExecution chre;

        public CryptoCallBack(HttpRequest hr, byte[] bytes, ClientHttpRequestExecution chre) {
            this.chre = chre;
            this.hr = hr;
            this.bytes = bytes;
        ...
        public ClientHttpResponse doWithRetry(RetryContext context) {
            ...
            if (context.getLastThrowable() != null) {
                log.debug("Eccezione di tipo " + context.getLastThrowable().getClass(), context.getLastThrowable());
            ...
            try {
                ...
                log.debug(hr.getMethod() + " # " + context.getRetryCount() + " a  [" + currentURI.toASCIIString()
                        + "]  ");
                ...
                if (!Arrays
                        .asList(HttpStatus.OK, HttpStatus.INTERNAL_SERVER_ERROR, HttpStatus.BAD_REQUEST,
                                HttpStatus.EXPECTATION_FAILED, HttpStatus.NOT_FOUND)
                        .contains(response.getStatusCode())) {
                    throw new RestClientException("Response code ottenuto " + response.getStatusCode()
